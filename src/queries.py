...
Uses the synthetic SQLite database generated by generate_mock_data.py.
"""
...
# Use the single source of truth for the DB path from config
DB_PATH = config.DB_PATH

def get_db_connection() -> sqlite3.Connection:
    # BUG FIX: Removed f"file:{DB_PATH}?mode=ro" and uri=True
    # This standard connection prevents Streamlit Cloud from crashing
    # if the file is slightly delayed in creation or locked.
...
def plan_fundraising_trip(target_state: str, limit: int = 20) -> dict:
    ...
    return {
        "results": results,
        "count": len(results),
        "summary": f"Found {len(results)} donors in {target_state.upper()} for trip planning.",
    }
